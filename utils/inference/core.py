...
def model_inference(full_frames: List[np.ndarray],
    ...
    for idx, (crop_frames, tfm_array, source_embed) in enumerate(zip(crop_frames_list, tfm_array_list, source_embeds)):
        ...
        if len(resized_frs) == 0:
            print(f"Warning: No valid frames after resizing for source {idx}")
            final_frames_list.append([])
            continue
            
        resized_frs = np.array(resized_frs)
        
        # Ensure resized_frs has correct shape (N, H, W, C)
        if resized_frs.ndim != 4:
            print(f"Warning: resized_frs has wrong dimensions: {resized_frs.shape}, expected 4D (N, H, W, C)")
            final_frames_list.append([])
            continue
