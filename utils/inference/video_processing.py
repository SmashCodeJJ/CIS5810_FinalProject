    ...
    for frame in tqdm(full_frames):
        try:
            ...
            if kps is None or len(kps) == 0:
                print(f"Warning: No keypoints detected in frame")
    ...
    for i, frame in tqdm(enumerate(full_frames)):
        for q in range (len(target_embeds)):  
            try:
                ...
                result = face_align.estimate_norm(kp, crop_size)
                ...
                if M is None:
                    print(f"Warning: Transformation matrix is None")
                    crop_frames[q].append([])
                    tfm_array[q].append([])
                    continue
                
                # Fix matrix shape if needed
                if M.shape != (2, 3):
                    if M.size == 6:
                        if M.shape == (3, 2):
                            M = M.T
                        elif M.shape == (6,) or M.shape == (1, 6) or M.shape == (3,):
                            M = M.reshape(2, 3)
                        else:
                            print(f"Warning: Cannot fix M shape {M.shape}")
                            crop_frames[q].append([])
                            tfm_array[q].append([])
                            continue
                    else:
                        print(f"Warning: M has wrong size {M.size}, expected 6")
                        crop_frames[q].append([])
                        tfm_array[q].append([])
                        continue
                ...
                print(f"Warning: Error in frame {i}, target {q}: {e}")
                crop_frames[q].append([])
                tfm_array[q].append([])     
